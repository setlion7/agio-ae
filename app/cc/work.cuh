...
__forceinline__ __device__ void
cc_compute(class gio *gp, vertex_t vid, vertex_t next_id, vertex_t *comp,
           bool *next_visit, uint32_t *changed) {
    vertex_t comp_src = comp[vid];
    vertex_t comp_next = comp[next_id];

    vertex_t comp_target;
    vertex_t next_target;
    ...
    if (comp_next != comp_src) {
        ...
        atomicMin((unsigned long long *)&(comp[next_target]), (unsigned long long)comp_target);
        next_visit[next_target] = 1;
        *changed = 1;
    }
}
...
work::build_iocb(class gio *gp, struct iocb *cb, uint64_t vid, uint32_t local_level) {
    ...
    force_assert_printf(data != nullptr, "%s: size: %lu\n", __func__, sizeof(*data));
...
__forceinline__ __device__ void
process_cc(class gio *gp, class work *wp, struct app_data *data, uint32_t *changed) {
    USER_TID;
    ...
    for (uint32_t i = 0 + lid; i < data->num_total_children; i = i + 32) {
        vertex_t vid = edge_ptr[i];

        cc_compute(gp, parent_vid, vid, wp->comp, wp->next_visit, changed);
    }
    __syncwarp();
}
...
work_main(class gio *gp, class work *wp,
    ...
    while (true) {
        ...
        if ((int64_t)ret_data >= 0) {
            process_cc(gp, wp, ret_data, changed_d);
            if (lid == 0) {
...
__global__ void
work_direct_process_only(class gio *gp, class work *wp, uint32_t *changed_d) {
    USER_TID;
    const uint32_t lid = tid % 32;
    class g_api g(gp);
...
__global__ void
update(class gio *gp, class work *wp) {
    force_assert(gridDim.x == 1);
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t stride = gridDim.x * blockDim.x;
    for (uint64_t i = tid; i < wp->vertex_count; i += stride) {
        wp->curr_visit[i] = 0;
    }
    __syncthreads();
    if (tid == 0) {
        bool *temp = wp->curr_visit;
        wp->curr_visit = wp->next_visit;
        wp->next_visit = temp;
    }
}
...
void work_launcher(nlohmann::json &out_js, const nlohmann::json &cuda_js,
    ...
    // const uint32_t n_blocks_process_only = n_blocks_req;
    ...
    do {
        ...
        void *work_direct_process_only_args[3] = {&(gp), &(wp), &changed_d};
        ...
        *changed_h = 0;
        ...
        // printf("%s: launch kernel n_blocks: %u, n_threads: %u, level: %u\n", __func__, n_blocks_req, n_threads, local_level);

    continue_level:
        *pending_io_h = 0;
        // printf("%s: launch app kernel n_blocks: %u, n_threads: %u, level: %u\n", __func__, n_blocks, n_threads, local_level);
